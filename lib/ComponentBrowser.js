...
function buildDoc(id,projData){
  //console.log('in buildDoc: ', id, projData);
  return new Promise((resolve, reject)=>{
    var doc = new Module({
      gitlabId: id,
      name: projData.name,
...
function updateModule(sw,doc){
  ...
  return new Promise((resolve, reject)=>{
    ...
    for(var key in sw.dependencies){
      //console.log(key);
      if(!latest.dependencies[key]){//added
        console.log('not found:', latest.dependencies[key], key);
        //changelog.push("A new module added" + key + "@" + sw.dependencies[key].version)
        var added = {}
        added[key] = sw.dependencies[key].version
        changelog.push({'added':added})
      } else if(sw.dependencies[key].version !== latest.dependencies[key]){//updated
        ...
        changelog.push({'updated':{'module': key, 'from': latest.dependencies[key], 'to': sw.dependencies[key].version}})
      }
      deps[key] = sw.dependencies[key].version;
    };

    for(var i in latest.dependencies){
      if(!sw.dependencies[i]){//removed
        //changelog.push("The module " + i + " has been removed")
        var removed = {}
        removed[i] = latest.dependencies[i]
        console.log(removed)
        changelog.push({'removed':removed})
      }
    };
      ...
      if(doc){//doc exists check if version exists
        ...
        getProjectByName(n).then((polarisModule)=>{
          ...
          getPackageJSONById(polarisModule.id).then((projectData)=>{// get proj json
            ...
            buildDoc(polarisModule.id,projectData).then((_doc)=>{
...
module.exports.process = function(shrinkwrapFile){
  return new Promise((resolve, reject)=>{
    if(!shrinkwrapFile){
      ...
      getPolarisModule(shrinkwrapFile).then((doc)=>{// get proj id
...
function getChangelog(mId){
  return new Promise((resolve, reject)=>{
    Module.findOne({gitlabId: mId}, function(err, doc){
      if(err){
        console.log('error finding module: ', err)
        reject(err)
        return
      }
      var changelog = {}
      if(doc && doc.versions){
        var latest = doc.versions[doc.versions.length - 1]
        var changelog = latest.changelog
        console.log('found changelog: ', changelog)
      }
      resolve(changelog)
    }).catch((e)=>{
      console.error(e);
...
module.exports.getChangelog = function(moduleId){
  return new Promise((resolve, reject)=>{
    getChangelog(moduleId).then((cl)=>{
      //console.log(doc)
      resolve(cl)
    }).catch((e)=>{
      console.error(e);
